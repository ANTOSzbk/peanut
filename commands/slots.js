...
exports.run = (client, message, args) => {
    ...
    if (!fs.existsSync('./jackpot.txt')) fs.writeFileSync('./jackpot.txt', 0);
    fs.readFile('./jackpot.txt', (err, data) => {
        if (err) throw err;
        let jackpot = parseInt(data, 10);
        if (!args.length) {
...
module.exports.desc = 'Zagraj w grę slots'
module.exports.args = 'ilość kredytów/all'