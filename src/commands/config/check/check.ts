  ...
  public async exec(message: Message) {
    ...
    return message.util?.send(
      new MessageEmbed()
        ...
        .addField(
          '❯ Mute Role',
          mute ? `${guild.roles.cache.get(mute)} \`✅\`` : '`❌`',
          true
        )
        .addField(
          '❯ Mod Log',
          modlog ? `${guild.channels.cache.get(modlog)} \`✅\`` : '`❌`',
          true
        )
        .addField(
          '❯ Guild Log',
          guildlogChannel
            ? `${guild.channels.cache.get(guildlogChannel)} \`✅\``
            : '`❌`',
          true
        )
        .addField(
          '❯ Member Log',
          memberlog.ID
            ? `${guild.channels.cache.get(memberlog.ID)} ${
                memberlog.MENTION ? '(w/ mention)' : ''
              } \`✅\``
            : '`❌`',
          true
        )
