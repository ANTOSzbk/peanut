...
export default class KickAction extends Action {
  ...
  public async exec() {
    ...
    const sentMessage = await this.message.channel.send(
      MESSAGES.ACTIONS.KICK.PRE_REPLY(this.member.user.tag)
    );

    try {
      try {
        await this.member.send(
          MESSAGES.ACTIONS.KICK.MESSAGE(guild.name, this._reason)
        );
      } catch {
        throw new Error(MESSAGES.ACTIONS.KICK.DM_BLOCKED(this.member.user.tag));
      }
      await this.member.kick(
        MESSAGES.ACTIONS.KICK.AUDIT(this.message.author.tag, totalCases)
      );
    } catch (error) {
